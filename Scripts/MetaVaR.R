...
MVC_list_smartbranching = tryParam (e, p, cov_1[,-1])

## Select maximum weigthed independant sets
MWIS_smartbranching = getMWIS (MVC_list_smartbranching)

## Filter loci to generate MVS
MVS_smartbranching = mvc2mvs(MWIS_smartbranching, freq = freq_1[,-1])
...
MWIS_list_smartbranching = getMWIS (MVC_list_smartbranching)

MVS_list_smartbranching = mvc2mvs(MWIS_list_smartbranching, freq = freq_1[,-1])

## store MVS in directory
writeMvsList (MVS_list_smartbranching, "MVS_name")



## Plot the MVS distribution of allele frequency, 1 can be replaced by the number of the MVC
plotMvs (MVS_smartbranching[[1]], type = "freq")

## Plot the MVS loci depth of coverage
plotMvs (MVS_smartbranching[[1]], type = "cov")

## Plot the pairwise FST matrix
plotMvs (MVS_smartbranching[[1]], type = "heatFst")

## Plot the expected and observed LK distribution
plotMvs (MVS_smartbranching[[1]], type = "LK")
...
print(MVS_smartbranching[[1]]@deg)
